...
query1;
	...
	if(end($update_p1)->name=='light_box_style'){
		$wpdb->query($sql_update_p1);
          ...
          $no_image = plugins_url().'/lightbox/images/No-image-found.jpg';
          $table_name = $wpdb->prefix . "hugeit_lightbox";
	$sql_update_p2 = <<<query2
	INSERT INTO `$table_name` (`name`, `title`, `value`) VALUES
	('watermark_width', 'Watermark width', '30'),
        ('lightbox_watermark_position', 'Lightbox watermark position', '3'),
        ('watermark_img_src', 'Watermark image source', '$no_image'),
        ('watermark_transparency', 'Watermark transparency', '100'),
        ('watermarket_image', 'Watermarket Image', 'false');

query2;

	$query2="SELECT name FROM ".$wpdb->prefix."hugeit_lightbox";
	$update_p2=$wpdb->get_results($query2);
